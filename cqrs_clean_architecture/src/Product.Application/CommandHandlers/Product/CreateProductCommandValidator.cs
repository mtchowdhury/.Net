...
public class CreateProductCommandValidator : AbstractValidator<CreateProductCommand>
{
    ...
    public CreateProductCommandValidator(IProductQueryRepository productQueryRepository)
    {
        ...
        RuleFor(p => p.EffectiveFrom).GreaterThan(DateTime.Today).WithMessage("Effective From Date can't be less than System Date");
        ...
        //RuleFor(p => p.EffectiveFrom.Date).Equal(p => p.EffectiveTo.Date).WithMessage("Effective From Date Can't be same as Effective To Date");
        //RuleFor(p => p.EffectiveFrom.Date).GreaterThan(p => p.EffectiveTo.Date).WithMessage("Effective From Date Can't be greater than Effective To Date");
        
        //RuleFor(p => p.EffectiveTo).GreaterThanOrEqualTo(p => p.EffectiveFrom).WithMessage("Effective From Date Can't be greater than or equal to Effective To Date");
        RuleFor(p => new { p.Name,p.ShortName,p.EffectiveFrom,p.EffectiveTo,p.ProductTypeId,p.CategoryId }).CustomAsync(async (property, context, cancellationToken) =>
        {
            ...
            if(!await _productQueryRepository.IsUniqueProductName(property.Name))
                context.AddFailure("Product name already exists");
            if (!await _productQueryRepository.IsValidDateRange(property.EffectiveFrom,property.EffectiveTo,property.ProductTypeId,property.CategoryId))
