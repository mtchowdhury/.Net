ammsAng.service('generalJournalService', ['$http', '$rootScope', function ($http, $rootScope) {
    ...
    this.getAHIF7List = function (filter) {
        return $http({
            method: 'POST',
            url: $rootScope.accountsApiBaseUrl + 'generalJournal/getAhif',
            data: filter,
            headers: $rootScope.headersWithoutLog
        });
    }

    this.getBalanceSheetReport = function (filter) {
        return $http({
            method: 'POST',
            url: $rootScope.programsApiBaseUrl + 'reports/balanceSheet/getBalanceSheet',
            data: filter,
            headers: $rootScope.headersWithoutLog
        });
    }
    ...
    this.getAdministrativeDistrictIdByBranchId = function (branchId) {
     ...
     this.updateReferenceVoucher = function (branchId, workingDate) {
         return $http({
             method: 'GET',
             url: $rootScope.accountsApiBaseUrl + 'generalJournal/updateJournals',
             params: {
                 branchId: Encrypt.encrypt(branchId),
                 workingDate: Encrypt.encrypt(workingDate)
