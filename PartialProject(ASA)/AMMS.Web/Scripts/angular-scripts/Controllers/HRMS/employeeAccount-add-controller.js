ammsAng.controller('employeeAccountAddController', ['$scope', '$rootScope', '$timeout', '$q', 'filterService', 'employeeAccountService', 'commonService', 'employeeService', 'hrmTransactionService',
    function ($scope, $rootScope, $timeout, $q, filterService, employeeAccountService, commonService, employeeService, hrmTransactionService) {
        $scope.eaccount = {};
        $scope.filters = {};
        $scope.filter = {};
       ...
       // delete $rootScope.branchInfo;
        ...
        $scope.loadEmployeesFromSp = function () {
            ...
            commonService.getEmployeeFilterFromSP($scope.filter.BranchId.Value, $rootScope.user.Role, $rootScope.user.EmployeeId, false, moment(new Date(1901,00,01)).format(), moment($rootScope.workingdate).format()).then(function (response) {
                $scope.EmployeeList = response.data.filter(e=>e.Name.includes("*"));
        ...
        $scope.addEmployeeAccount = function () {
            ...
            if ($scope.checkIfEmployeeIsInTransferState($scope.eaccount.EmployeeId)) return;
