ammsAng.controller('branchDataUploadController', [
    ...
    function ($scope, $rootScope, commonService, documentService, DTOptionsBuilder, DTColumnDefBuilder) {
       ...
       DTColumnDefBuilder.newColumnDef(4).notSortable()
        ...
        $scope.upload = function () {
            ...
            function (isConfirmed) {
                if (isConfirmed) {
                    $scope.file = $scope.files[$scope.files.length - 1];
                    $scope.uploadStartTime = moment().format();
                    documentService.uploadBranchData($scope.file, $scope.selectedBranchId, $rootScope.workingdate, $scope.file.name).then(function (response) {
                        if (response.data.Success) {
                            ...
                            documentService.updateUniversalFileUploadTime(timeobject).then(function (res) {
                                if (!res.data.Success) {
        ...
        $scope.getLocalBranchData = function () {
            ...
            documentService.getUniversalBranchData($scope.selectedBranchId, $scope.roleId).then(function (response) {
