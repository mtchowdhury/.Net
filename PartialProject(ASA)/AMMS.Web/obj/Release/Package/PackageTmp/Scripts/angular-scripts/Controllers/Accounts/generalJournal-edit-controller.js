ammsAng.controller('generalJournalEditController', ['$scope', '$rootScope', 'commonService', '$timeout', 'employeeService', 'DTOptionsBuilder', 'DTColumnDefBuilder', 'generalJournalService',
    function ($scope, $rootScope, commonService, $timeout, employeeService, DTOptionsBuilder, DTColumnDefBuilder, generalJournalService) {
       ...
       // $scope.voucher.TransactionDate = moment($rootScope.branchWorkingDay).format('DD-MM-YYYY');
        ...
        $scope.getVoucherInfo=function() {
            generalJournalService.getVoucherById($rootScope.selectedJournalObject.Id).then(function (response) {
                ...
                $scope.voucher[0].GlTransactions.forEach(function(tr,i) {
                    ...
                    //tr.BankAccountName = tr.BankAccount;
                ...
                //groupby voucher no in history table
                $scope.trs = [];
                $scope.uniqueTrhistories = Array.from(new Set($scope.voucher[1].GlTransactions.map(o => o.VoucherNumber)));
                $scope.uniqueTrhistories.forEach(function(tr) {
                    $scope.trs.push($scope.voucher[1].GlTransactions.filter(gtr=>gtr.VoucherNumber === tr));
                });
            });
        }
                ...
                swal({
                   ...
                   // text: $rootScope.showMessage($rootScope.editConfirmation, $rootScope.GeneralJournal),
                    ...
                    if (isConfirmed) {
                        ...
                        generalJournalService.editGeneralJournal($scope.voucher).then(function (response) {
                            if (response.data.Success) {
                                ...
                                swal("Voucher is edited with code:  " + $scope.voucher[0].VoucherNo + " with Amount: " + $scope.debitSum);
                                $timeout(function () { $scope.clearModelData(); }, 300);
