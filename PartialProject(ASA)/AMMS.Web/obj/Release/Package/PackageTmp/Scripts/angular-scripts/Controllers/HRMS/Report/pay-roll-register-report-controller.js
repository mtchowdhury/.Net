ammsAng.controller('payRollRegisterReportController', ['$scope', '$rootScope', 'filterService', '$timeout', 'reportService', 'DTOptionsBuilder', 'DTColumnDefBuilder',
    function ($scope, $rootScope, filterService, $timeout, reportService, DTOptionsBuilder, DTColumnDefBuilder) {
       ...
       DTColumnDefBuilder.newColumnDef(4).notSortable()
        ...
        $scope.getFilters = function (reportId) {
            ...
            reportService.getFilters(reportId, $scope.roleId, $scope.employeeId, $scope.branchId, moment($rootScope.workingdate).format(), moment($rootScope.workingdate).format()).then(function (response) {
                $scope.filters = angular.copy(response.data);
                $scope.filters.forEach(function (filterValue) {
                    if (filterValue.FilterType === "DropDown") {
                        var hasAll = filterValue.Dropdowns.filter(f => f.Value === '-100000');
                        filterValue.Param = hasAll.length > 0 ? '-100000' : filterValue.Dropdowns.length > 0 ? filterValue.Dropdowns[0].Value : '';
