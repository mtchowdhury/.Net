        ...
        return {
            ...
            link: function (scope, element, attrs, fn) {
                ...
                function generateErrorMessage(messageText) {
                    ...
                    //validation message from group transactions are sent with strating "$$$" . 
                    //Check if the are from group transaction , If so, put appropriate css class 

                    var fromTransaction = (scope.$eval(messageText).substring(0, 3) === "$$$") ? true : false;
                    var fromTransactionMessage = scope.$eval(messageText).substring(3);
                    return !fromTransaction
                        ? "<label class='control-label has-error validationMessage'>" + scope.$eval(messageText) + "</label>"
                        : "<label class='control-label has-error validationGroupTransctionMessage'>" + fromTransactionMessage + "</label>";
                }
                ...
                function isValidationMessagePresent(element) {
                    ...
                    for (var i = 0; i < elementSiblings.length; i++) {
                        if (angular.element(elementSiblings[i]).hasClass("validationMessage") || angular.element(elementSiblings[i]).hasClass("validationGroupTransctionMessage")) {
