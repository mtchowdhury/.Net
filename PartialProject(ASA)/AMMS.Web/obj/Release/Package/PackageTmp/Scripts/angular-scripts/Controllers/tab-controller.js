ammsAng.controller('tabController', ['$scope', '$rootScope', '$http', 'authService', 'tabService', 'branchService', 'documentService', '$timeout',
    function ($scope, $rootScope, $http, authService, tabService, branchService, documentService, $timeout) {
        ...
        $scope.getMenus = function () {
            ...
            tabService.getMenus($rootScope.user)
      .success(function (s) {
          $scope.menus = s;
          for (let i = 0; i < $scope.menus.length; i++) {
              $scope.changeModule($scope.menus[i], null, $rootScope.Modules.PROGRAM_OFFICER, $scope.officer);
          };
          branchService.getGroupTypes().then(function (response) {
              for (let i = 0; i < $scope.menus.length; i++) {
                  $scope.changeModule($scope.menus[i], null, $rootScope.Modules.GROUP_TYPE, JSON.parse(response.data));
              }
              branchService.getGroups($rootScope.selectedBranchId).then(function (response) {                 
                  ...
                  for (var j = 0; j < JSON.parse(response.data).length; j++) {
                      ...
                      $scope.groupNames.push({ Name: JSON.parse(response.data)[j].Name, Value: JSON.parse(response.data)[j].Id, ProgramOfficerId: JSON.parse(response.data)[j].ProgramOfficerId, GroupTypeId: JSON.parse(response.data)[j].GroupTypeId });
                  }
                  for (let i = 0; i < $scope.menus.length; i++) {
                      $scope.changeModule($scope.menus[i], null, $rootScope.Modules.GROUP, JSON.parse(response.data));
                  }                 
                  branchService.getMembersWithScheduleColor($scope.groupIds, $rootScope.workingdateInt, $rootScope.selectedBranchId).then(function (response) {                     
                      for (let i = 0; i < $scope.menus.length; i++) {
                          $scope.changeModule($scope.menus[i], null, $rootScope.Modules.MEMBER, JSON.parse(response.data));
      ...
      //console.log(data, status);
        ...
        $scope.changeModule = function (module, parent, targetModuleId, dataToBeInserted) {
            ...
            if (module && module.Module === targetModuleId) {
                ...
                if (flag === 1) parent.SubModules = changedData;
        ...
        $scope.openNewTab = function (menu) {
            ...
            $scope.propertyData = [];
            //menu.id = menu.Module + '_' + menu.DisplayName.replace(/./g, '_').replace(/ /g, '_');
                ...
                $scope.propertyData[menu.id].success(function (data) {
                    ...
                    if (menu.ModuleId == $rootScope.Modules.MEMBER) {
                        ...
                        if (menu.GroupTypeId == $rootScope.GroupTypes.BAD_DEBT) {
                            ...
                            //    return item.PropertyUniqueId !== $rootScope.Properties.LOAN_ACCOUNT;
                            //});
                            data = data.filter(function (item) {
                            ...
                            //data = data.filter(function (item) {
                            //    return item.PropertyUniqueId !== $rootScope.Properties.CHANGE_PROGRAM_GROUP_TYPE;
                            //});
        ...
        $scope.switchTab = function (id) {
            //console.log($scope.tabs, id, $('#tab a[href="#tabContent_' + id + '"]').tab());
            // $scope.selectedMenu = $scope.tabs.find(e => (e.ModuleId + '_' + e.Name) == id)[0];
        ...
        $scope.labelStyle = function (level, hasMeetingDay, hasParentMeetingDay, hasSchedule) {
            ...
            if (hasMeetingDay && level === 4)
                style["color"] = "#0061FF";
            else if (hasMeetingDay && level === 5 && hasParentMeetingDay)
                style["color"] = "#0061FF";
            else if (level === 5 && hasSchedule)
                style["color"] = "#B12A54";
            return style;
        }
        ...
        $scope.removeAllTab = function () {
            ...
            for (var i = 0; i < k; i++) {
                ...
                if (tab && tab.ConfirmPrompt && tab.FormType !== "VIEW") { p++; continue; }
                if (tab) {
                    $scope.tabs.splice($scope.tabs.indexOf(tab), 1);
                    $scope.openWindows.splice($scope.openWindows.indexOf(tab.id), 1);
                }
        ...
        $scope.removeAllTabinDayOpenClose = function () {
            ...
            for (var i = 0; i < k; i++) {
                var tab = $scope.tabs[p];
                //if (tab && tab.ConfirmPrompt) { p++; continue; }
                if (tab) {
                    $scope.tabs.splice($scope.tabs.indexOf(tab), 1);
                    $scope.openWindows.splice($scope.openWindows.indexOf(tab.id), 1);
                }
        ...
        $rootScope.$on('propertyNotificationClicked', function (args, newTabInfo) {
            tabService.getProperties($rootScope.user.UserId, newTabInfo.ModuleId).success(function (data) {
                ...
                var tabData = { id: newTabInfo.ModuleId };
        ...
        $rootScope.$on('commandNotificationClicked', function (args, newTabInfo) {
            ...
            $rootScope.notificationData = (newTabInfo.OtherInfoJson != null) ? JSON.parse(newTabInfo.OtherInfoJson) : {};
            var menu = $scope.menus.filter(e => e.ModuleId = newTabInfo.ModuleId)[0];
            // yet to be implemented in case of any command type of notification. according to requirment yet no module have this type of notificaiton.
