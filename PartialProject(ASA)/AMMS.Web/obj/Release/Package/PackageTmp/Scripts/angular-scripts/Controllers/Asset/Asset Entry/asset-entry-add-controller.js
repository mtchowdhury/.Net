ammsAng.controller('assetEntryAddController', ['$scope', '$rootScope', 'assetEntryService', 'commonService', 'filterService', '$timeout', 'employeeService', 'loanGroupService', 'documentService', 'assetTypeService', 'assetCategoryService', 'DTOptionsBuilder',
    function ($scope, $rootScope, assetEntryService, commonService, filterService, $timeout, employeeService, loanGroupService, documentService, assetTypeService, assetCategoryService, DTOptionsBuilder) {
        ...
        $scope.filterCategory = function (itemId) {
            ...
            if ($scope.asset.ItemTypeId) {
                assetEntryService.getAssettag($rootScope.selectedBranchId, moment($rootScope.workingdate).format(), $scope.asset.ItemTypeId).then(function (response) {
                    $scope.asset.AssetId = response.data;
                });
            }
            if ($scope.asset.ItemTypeId !== undefined)
                assetTypeService.getAssetItem($scope.asset.ItemTypeId).then(function (response) {
        ...
        $scope.changeLocationOffice = function () {
            assetEntryService.getEmployeeByOfficeCode($scope.asset.LocationOfficeId).then(function (response) {
        ...
        $scope.addAssetEntry = function () {
            ...
            //var cat = $scope.ItemList.  $scope.asset.ItemCategoryId
            assetCategoryService.getCategoryInfo($scope.asset.ItemCategoryId).then(function (response) {
                var res = response.data;
                if (res.Status == 2) {
                    swal("Inactive Category");
                    return;
                }
            });
            ...
            $scope.asset.AssetType = $scope.asset.AssetTypeId;
            ...
            swal({
                title: $rootScope.showMessage($rootScope.addConfirmation, $rootScope.AssetEntry),
                    ...
                    if (isConfirmed) {
                        ...
                        if ($scope.files.length > 0) {
                            documentService.uploadAsset($scope.files, $rootScope.user.UserId,
                                ...
                                .then(function (res) {
                                    if (res.data.Success) {
                                        assetEntryService.add($scope.asset).then(function (response) {
                                            if (response.data.Success) {
                                                ...
                                                assetEntryService.addAssetDocuments($scope.assetDocuments).then(function (r) {
                                                    if (r.data.Success) {
                                                        ...
                                                        swal($rootScope.showMessage($rootScope.addSuccess, $rootScope.AssetEntry), "Successful!", "success");
                            ...
                            $scope.asset.Mushak11 = $scope.asset.Mushak11Radio;
                            assetEntryService.add($scope.asset).then(function (response) {
                                if (response.data.Success) {
                                    $rootScope.$broadcast('asset-add-finished');
                                    swal($rootScope.showMessage($rootScope.addSuccess, $rootScope.AssetEntry), "Successful!", "success");
        ...
        $scope.chalanDateValidator = function () {
            ...
            if ($scope.asset.ChalanDate != undefined && moment($scope.asset.ChalanDate).add(1, 'days') < moment($scope.asset.PurchaseWorkingDate)) {
